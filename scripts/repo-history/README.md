...
```sh
git ls-tree -r HEAD --format='%(objectname) %(path)' | python3 scripts/repo-history/analyze-duplication.py
git ls-tree -r HEAD --format='%(objectname) %(path)' | python3 scripts/repo-history/analyze-variants.py
```
