...
userinit(void)
{
  ...
  inituvm(p->pagetable, p->asid, _binary_initcode_start, (int)_binary_initcode_size);
  p->sz = PGSIZE;
...
growproc(int n)
{
  ...
  sz = proc->sz;
  if(n > 0){
    if((sz = allocuvm(proc->pagetable, proc->asid, sz, sz + n)) == 0)
      return -1;
  } else if(n < 0){
...
fork(void)
{
  ...
  if((np->pagetable = copyuvm(proc->pagetable, np->asid, proc->sz)) == 0){
    kfree(np->kstack);
    np->kstack = 0;
    np->state = UNUSED;
    return -1;
