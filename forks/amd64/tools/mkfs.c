...
main(int argc, char *argv[])
{
  ...
  for(i = 2; i < argc; i++){
    ...
    // tests/, so open() needs the full path while the directory entry needs
    // the basename. Split them. The assert now checks the basename and so
    // always holds, but documents the invariant DIRSIZ relies on.
