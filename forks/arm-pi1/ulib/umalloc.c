...
//
// Confirmed by tracing on arm-pi1-bis - this fork's own identical
// twin, same CPU/float-ABI pair, same shared shells/sh.c and umalloc.c:
// its shell crashed on the very first malloc() any forked child ever
// makes (shells/sh.c's parsecmd()/execcmd(), building the parsed
// command tree), getting only as far as morecore()'s own entry before
// dying, i.e. inside the "nu % nunits" rounding computation itself,
// before ever reaching sbrk(). This fork's own CI failures (GitHub
// Actions: "usertests" faulting at a garbage pc in the same
// 0x10b17xx-0x10b18xx range) are the same bug - it just never
// reproduced locally here the way it did on arm-pi1-bis, most likely
// timing-sensitive in exactly which iteration first calls malloc().
// forks/arm (real ARMv7-A + Thumb-2, per its own "-march=armv7-a") and
...
// this plain-ARMv6 pair cannot run it. See notes_arch_arm_pi1_bis.txt
// and notes_arch_arm_pi1.txt.
//
