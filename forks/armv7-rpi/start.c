...
// confirmed via the QEMU monitor's "info qtree" that "-M raspi2b"'s
// emulated Mini-UART ("bcm2835-aux") has NO chardev backend at all
// (chardev="", same finding as the sibling forks/rpi1 port's own bring-
// up - see notes_arch_rpi1.txt), while the PL011 genuinely is wired to
// "-serial"/stdio (chardev="serial0") - nothing written to the Mini-
// UART's registers is observable no matter how correct the driver is.
// This fork has no real hardware of its own to protect (unlike rpi1/
// rpi2 - see notes_arch_armv7_rpi.txt), so switching the actual
// peripheral target is safe. GPIO 14/15 need ALT0 for PL011 (not
// ALT5, the Mini-UART's own alternate function).
void _uart_init ( void )
{
  unsigned int ra;

  // Disable UART0 before reconfiguring it.
  write32(UART0_CR,0);

  // Setup the GPIO pin 14 && 15 for UART0 (ALT0).
  ra=read32(GPFSEL1);
  ra&=~(7<<12); //gpio14
  ra|=4<<12;    //alt0
  ra&=~(7<<15); //gpio15
  ...
  write32(UART0_ICR,0x7FF);   // Clear pending interrupts.
  write32(UART0_IBRD,1);      // Baud rate divisor (QEMU's PL011 model
  write32(UART0_FBRD,40);     // doesn't enforce real timing).
  write32(UART0_LCRH,(1<<4)|(3<<5)); // FIFO enable, 8 data bits.
  write32(UART0_CR,(1<<0)|(1<<8)|(1<<9)); // UARTEN, TXE, RXE.
}

void _uart_putc ( unsigned int c )
{
  while(read32(UART0_FR)&(1<<5)) ; // wait while TXFF (transmit FIFO full)
  write32(UART0_DR,c);
}
