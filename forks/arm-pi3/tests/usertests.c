  ...
  if((pid = fork()) == 0){               //The parent process executes the test.
    ...
    while((m2 = malloc(10001)) != 0){    // Allocate ~10kb to m2.
      *(char**)m2 = m1;                  // Set m2 to point to m1.
      ...
      blocks++;
      if (blocks >= 0x147A) {
	printf("Tested first 50MB. Breaking early for fast testing.\n");
	m2 = 0;
	break;
      }
    }
...
main(int argc, char *argv[])  
{
  ...
  if (test_mem) {
    ...
    printf("Skipping memory test because the test is slow. Run 'usertests m' to test the memory\n");
  }
  ...
  if (test_preempt) {
    ...
    printf("Skipping preempt test: Test is designed for two or less cores. Run 'usertests p' to test preemption\n");
  }
