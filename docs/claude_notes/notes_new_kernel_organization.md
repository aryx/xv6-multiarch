...
- **`sleep_release(chan, lk)`** (`kernel/arch/<arch>/arch_proc.h`) -
  ...
  a real fix for a narrow lost-wakeup race between releasing the
  caller's lock and the process actually blocking. `sleep_release()`
...
lost-wakeup race in that scheduler -> `sleep_release` is just `sleep`).
